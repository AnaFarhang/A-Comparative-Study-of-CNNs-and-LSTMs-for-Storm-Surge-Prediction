...
for prediction_duration in [12, 24]:
#for prediction_duration in [12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144, 156, 168, 180]:
  ...
  if build_train_set == 1:
      ...
      while True:
          ...
          if np.max(w_indices) >= wind['t'].shape[0]:
              break
          # For the XAI part, here we can simply remove the features that we want except tide. For tide, there is a separate file
          # As an example, we can remove wind['u'][indices] and have: x1.append(np.stack((wind['v'][indices], wind['p'][indices]), axis=-1))
  ...
  #x2 sample data for tide
  #x1 and x2 training data
  #y is the lable
  ...
  for j in range(n_holdout):
      ...
      input_tide = Input(shape=(n_timesteps, 1))
      x = Concatenate()([x, input_tide])
      x = Dense(32, activation='relu')(x)
      x = Dropout(0.2)(x)
      x = Dense(1, activation='linear')(x)
      
      model = Model(inputs=[input_wind, input_tide], outputs=x)
      model.summary()
      model.compile(
      ...
      hist = model.fit([x_train, x2_train], y_train,
      ...
      test_vals = model.evaluate([x_test, x2_test], y_test, batch_size=64, verbose=1)
